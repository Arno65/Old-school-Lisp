...
;;  version 0.03b   2026-01-23    The common lisp version
;; 
...
;;
(defun move-to-list (move)
  (let ((move-as-list (coerce move 'list)))
    (if (= (length move-as-list) 4)
        (list (list (- (char-code (car    move-as-list)) 96)
                    (- (char-code (cadr   move-as-list)) 48))
              (list (- (char-code (caddr  move-as-list)) 96)
                    (- (char-code (cadddr move-as-list)) 48)))
        '())))

(defun moves-to-lists (olsl)
  (mapcar #'move-to-list olsl))

;; ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ---
;; The actual opening library in readable string format
;;

(defconstant opening-library-strings-white
...
(defconstant opening-library-strings-black
...
(defconstant opening-library-white
  (mapcar #'moves-to-lists opening-library-strings-white))

(defconstant opening-library-black
  (mapcar #'moves-to-lists opening-library-strings-black))
